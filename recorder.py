        ...
        try:
            async with websockets.connect(
                ...
                try:
                    ...
                    # Without this a server that never sends EndOfTranscript
                    # hangs the run forever, and `make sweep` along with it.
                    failed = f"no EndOfTranscript within {clip_len + GRACE:.0f}s"
                    done.set()
        except websockets.exceptions.ConnectionClosed as exc:
            # The server can reject a connection at the protocol level (an
            # auth failure closing with 4001, say) rather than sending an
            # application-level Error message - that raises here, outside
            # receive()'s own Error handling, and without this the run ends
            # as an uncaught traceback with nothing written to out_path at
            # all. A failed run must look failed - see docs/DOD.md R6 - not
            # crash with no evidence.
            failed = f"connection closed: {exc}"
            log("Error", {"message": "Error", "type": "connection_closed",
                           "reason": str(exc)})
